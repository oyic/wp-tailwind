...
<form name="getpdf" action="<?php echo admin_url('admin-post.php?'.time())?>" method="POST">
		<?php wp_nonce_field( 'check_pdf_submit', 'my_pdf_nonce_field' ); ?>
		<input type="hidden" name='action' value="printPDFFile">
		<input type="hidden" name="slug" value="<?php echo $post->post_name ?>">
		<input type="hidden" name="type" value="<?php echo $post->post_type ?>">
		<button type="submit" class="button ">
			<!-- <svg class="svg-inline--fa fa-file-pdf fa-w-12" aria-hidden="true" data-prefix="far" data-icon="file-pdf" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" data-fa-i2svg="">
			...
			</svg> --><!-- <i class="far fa-file-pdf"></i> --> <?php echo $button_label?:'Download PDF'; ?>
		</button>
	</form>